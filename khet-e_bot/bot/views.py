...
disease_model_path = 'bot/plant_disease_model.pth'
disease_model = ResNet9(3, len(disease_classes))
disease_model.load_state_dict(torch.load(
    disease_model_path, map_location=torch.device('cpu')))
disease_model.eval()

crop_recommendation_model_path = 'bot/RandomForest.pkl'
...
def predict_image(url, model=disease_model):
    """
...
def webhook(request):
    ...
    if request.method == "POST":
        ...
        media_msg = request.POST.get('NumMedia')
        print(media_msg)
        
        if media_msg == '1':
        	pic_url = request.POST.get('MediaUrl0')
            print(pic_url)
        	prediction = predict_image(pic_url)
        	translation = translator.translate(str(disease_dic[prediction]).replace("<br/>", "").replace("<i>","").replace("</i>","").replace("<b>","").replace("</b>",""), dest=main_lang[0])
        	response.message(translation.text)

        else:
	        lang = translator.detect(message).lang
	        if main_lang[0] != lang and main_lang[0] == 'en':
	        	main_lang[0] = lang
	        if main_lang[0] != lang and lang != 'en':
	        	main_lang[0] = lang
	        mess_trans = translator.translate(message)
	        print(mess_trans.text)
	        if mess_trans.text.lower() in ["1.", "1", "Crop", "Crop Prediction", "First"]:
	        	translation = translator.translate("Enter info like - \nCrop input = Nitrogen,Phosphorous,Potassium,pH,Rainfall,State,City\nFor example - \nCrop input = 25,25,50,7,100,Uttar Pradesh,Lucknow", dest=main_lang[0])
	        	response.message(translation.text)

	        elif mess_trans.text.startswith("Crop input = "):
	        	val = mess_trans.text[mess_trans.text.index("= ")+2:].split(",")
	        	print(val)
	        	if len(val) != 7:
	        		translation = translator.translate("Incorrect Format, Enter info like - \nCrop input = Nitrogen,Phosphorous,Potassium,pH,Rainfall,State,City\nFor example - \nCrop input = 25,25,50,7,100,Uttar Pradesh,Lucknow", dest=main_lang[0])
	        		response.message(translation.text)
	        	else:
	        		N = int(val[0])
			        P = int(val[1])
			        ...
			        if weather_fetch(city) != None:
			        	...
			        	translation = translator.translate("Crop Prediction for you is = " + final_prediction, dest=main_lang[0])
			        	response.message(translation.text)
			        else:
			        	translation = translator.translate("Can't predict, Try Again!", dest=main_lang[0])
			        	response.message(translation.text)

	        elif mess_trans.text.lower() in ["2.", "2", "Fertilizer Prediction", "Fertilizer", "Second"]:
	        	translation = translator.translate("Enter info like - \nFertilizer input = Crop name,Nitrogen,Phosphorous,Potassium\nFor example - \nFertilizer input = Pomegranate,25,25,50", dest=main_lang[0])
	        	response.message(translation.text)

	        elif mess_trans.text.startswith("Fertilizer input = "):
	        	val = mess_trans.text[mess_trans.text.index("= ")+2:].split(",")
	        	print(val)
	        	if len(val) != 4:
	        		translation = translator.translate("Incorrect Format, Enter info like - \nFertilizer input = Crop name,Nitrogen,Phosphorous,Potassium\nFor example - \nFertilizer input = Pomegranate,25,25,50", dest=main_lang[0])
			        ...
			        if max_value == "N":
			            ...
			            if k < 0:
			                ...
			                key = "Klow"
			        translation = translator.translate(str(fertilizer_dic[key].replace("<br/>", "").replace("<i>","").replace("</i>","")), dest=main_lang[0])
			        response.message(translation.text)

			
	        elif mess_trans.text in ["3.", "3", "Disease Prediction", "Disease", "Third"]:
	        	translation = translator.translate("Please send image of crop/plant to get disease prediction", dest=main_lang[0])
	        	response.message(translation.text)

	        elif mess_trans.text.startswith("Change language = ") or mess_trans.text.startswith("4") or mess_trans.text.startswith("4."):
	            lang = translator.detect(mess_trans.text[mess_trans.text.index("= ")+1:]).lang
	            main_lang[0] = lang
	            translation = translator.translate("Language changed to = " + main_lang[0], dest=main_lang[0])
	            response.message(translation.text)

	        else:
	        	translation = translator.translate("What do you want to know? \n1. Crop Prediction \n2. Fertilizer Prediction \n3. Disease Prediction\n4. Change Language = <lang>", dest=main_lang[0])
	        	response.message(translation.text)
